...
impl KeyringStore {
    ...
    pub fn save_token_with_app<R: Runtime>(
        ...
        app.keyring().set_password(Self::SERVICE_NAME, &key, token)?;
    ...
    pub fn get_token_with_app<R: Runtime>(
        ...
        let key = format!("{}_token", platform);
        let token = app.keyring().get_password(Self::SERVICE_NAME, &key)?
    ...
    pub fn has_token_with_app<R: Runtime>(
        app: &AppHandle<R>,
        platform: &str,
    ) -> bool {
    ...
    pub fn save_oauth_secret_with_app<R: Runtime>(
        ...
        app.keyring().set_password(Self::SERVICE_NAME, &key, secret)?;
        
        eprintln!("[KeyringStore] OAuth secret saved for platform: '{}'", platform);
        Ok(())
    }
    ...
    pub fn get_oauth_secret_with_app<R: Runtime>(
        ...
        let key = format!("{}_oauth_secret", platform);
        let secret = app.keyring().get_password(Self::SERVICE_NAME, &key)?
