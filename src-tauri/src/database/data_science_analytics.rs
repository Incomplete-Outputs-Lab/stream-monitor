...
/// Phase 3: Get retention cohort analysis
pub fn get_retention_cohort(
    conn: &Connection,
    channel_id: i64,
    _start_time: Option<&str>,
    _end_time: Option<&str>,
) -> Result<RetentionCohortResult, duckdb::Error> {
    // Simplified retention analysis - cohort by week
    let sql = r#"
        WITH first_seen AS (
            SELECT 
                user_name,
            ...
            FROM chat_messages cm
            LEFT JOIN streams s ON cm.stream_id = s.id
            WHERE (cm.channel_id = ? OR s.channel_id = ?)
            GROUP BY user_name
        ),
        user_activity AS (
            SELECT 
                cm.user_name,
                DATE_TRUNC('week', cm.timestamp) as activity_week
            FROM chat_messages cm
            LEFT JOIN streams s ON cm.stream_id = s.id
            WHERE (cm.channel_id = ? OR s.channel_id = ?)
            GROUP BY cm.user_name, activity_week
        )
        SELECT 
            fs.cohort_week::VARCHAR as cohort,
            COUNT(DISTINCT fs.user_name) as cohort_size
        FROM first_seen fs
    ...
    let mut stmt = conn.prepare(sql)?;
    let cohorts: Vec<RetentionCohort> =
        utils::query_map_with_params(&mut stmt, &params, |row| {
            Ok(RetentionCohort {
                cohort_week: row.get(0)?,
                cohort_size: row.get(1)?,
                week_0: 100.0, // Week 0 is always 100%
                week_1: 0.0,   // Would need more complex calculation
                week_2: 0.0,
                week_3: 0.0,
                week_4: 0.0,
            })
    ...
    Ok(RetentionCohortResult {
        cohorts,
        overall_retention: 0.0, // Would calculate from cohorts
    })
}
